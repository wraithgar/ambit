...
function parseQuarters(tokens) {
    var parsed, quarter, year, guessYear;
    //Needs to end in a season or a season/year
    year = tokens.slice(-1)[0];
    if (year.match(YEAR_MATCH)) {
        quarter = tokens.slice(-2)[0];
        if (year < 100) {
            year = year + 2000;
        }
    } else {
    ...
    parsed = {
        start: moment(new Date(year, quarter.month -1 , quarter.day)),
        end: moment(new Date(year, quarter.month -1, quarter.day))
    };
    return parsed;
}
