...
//Removes deprication warning since we're literally in the one
//use case they describe as valid for doing this
...
function parseYear(tokens) {
  var startDate, endDate;
  var year = _.find(tokens, function (token) { return token.match(YEAR_MATCH); });
  if (!year) {
    return;
  }
  if (year < 100) {
...
function parseMonth(tokens, now) {
  ...
  if (tokens.length === monthIndex + 1) {
    ...
    year = _.find(tokens, function (token) { return token.match(YEAR_MATCH); });
    if (year < 100) {
      year = Number(year) + 2000;
    }
  }
